$(document).ready(function () {
  ...
  $(".ptn_pop_cancel_btn").on("click", function () {
    ...
    $("#ptn_pop_name").val('').addClass("ptn_pop_input").removeClass("ptn_err_input mr");
    $("#ptn_pop_tel").val('').addClass("ptn_pop_input").removeClass("ptn_err_input");
  ...
  const nameCheck = () => {
    ...
    if (!$('#ptn_pop_name').val()) {
      ...
      $('#ptn_err_name').css({ display: 'block' }).text('파트너명을 입력해주세요');
      return false;
    } else if (name_len < 2) {
      $('#ptn_pop_name').addClass('ptn_pop_err_input ptn_pop_mr').focus();
      $('#ptn_pop_name').removeClass('ptn_pop_input');
  ...
  const telCheck = () => {
    if (!$('#ptn_pop_tel').val()) {
      ...
      $('#ptn_err_tel').css({ display: 'block' }).text('업체 연락처를 입력해주세요');
      return false;
  ...
  $('.ptn_pop_term').find('input').on('change', function () {
